...
import me.joba.pathtracercluster.pathtracer.render.Tracer.Photon;
...
//        y = y * (2.0 / (maxY - minY)) - minY - 1;
        ...
        buffer[i11] = buffer[i11].add(cie.scale(c11));
        buffer[i12] = buffer[i12].add(cie.scale(c12));
        buffer[i21] = buffer[i21].add(cie.scale(c21));
        buffer[i22] = buffer[i22].add(cie.scale(c22));
    }
    
    public void plot(List<Photon> photons) {
        lock.lock();
        try {
            for(Photon photon : photons) {
                Vector3 cie = getCIEVector(photon.wavelength);
    ...
    private Vector3 getCIEVector(double wavelength) {
        double indexf = (wavelength - 380) / 5;
        int index = (int)indexf;
        double remainder = indexf - index;
        if(index < -1 || index > 80) {
            return new Vector3(0, 0, 0);//Wavelength invisible
        }
        else if(index == -1) {
            return new Vector3(
                    CIE_X[0] * remainder, 
